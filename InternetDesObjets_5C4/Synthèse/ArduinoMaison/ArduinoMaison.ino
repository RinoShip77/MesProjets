...
String lcd_char = "";  //Déclare une variable pour contenir les lettres a afficher sur l'écran LCD
...
void pwm_control() {
  switch (val) {
    ...
    case 'x':  //Si on entre la lettre 'x', ...
      if (mega.available() > 0) { //On vérifie que la console (du MÉGA) est disponible (si elle elle n'est pas vide)
        Serial.print("BLE "); //On affiche l'origine de la commande
        lcd_char = mega.readStringUntil('#'); //On va lire la console jusqu'à ce qu'on atteingne un dièse
      } else if (Serial.available() > 0) { //On vérifie que notre console (de la maison) est disponible (si elle elle n'est pas vide)
        Serial.print("Serial "); //On affiche l'origine de la commande
        lcd_char = Serial.readStringUntil('#'); //On va lire la console jusqu'à ce qu'on atteingne un dièse
      }
      commandeVal = -1;
      mylcd.clear();
      mylcd.setCursor(0,0);
      mylcd.print(lcd_char.substring(0,17)); //Inscrit la commande sur l'écran LCD
      break;  //exit loop
    default:
      break;
  }
